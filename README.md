...
Все операции только читают и перемещают. **Удаления писем нет вообще** —
это сознательное решение: разгребать почту безопаснее перемещением.
...
**Нет отправки писем.** Сервер умеет только читать и перемещать, поэтому
скомпрометированный контекст не может разослать что-то от вашего имени.
